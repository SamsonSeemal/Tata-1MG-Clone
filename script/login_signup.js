...
// signup
let register_data;
let Registration_data = JSON.parse(localStorage.getItem("register_data")) || [];
function register() {
  let mobile = document.querySelector("#mobile_number").value;
  let password = document.querySelector("#one_time_p").value;
  let email = document.querySelector("#email_new").value;
  ...
  Registration_data.push(register_data);
  alert("Successful Sign UP");
  window.location.href = "login.html";
  localStorage.setItem("register_data", JSON.stringify(Registration_data));


  // flag = false;
  // for (let i = 0; i < Registration_data.length; i++) {

  //   if (Registration_data[i].mobile == mobile && Registration_data[i].password == password && Registration_data[i].email == email) {
  //     flag = true;
  //   }
  //   else {
  //     flag = false;
  ...
  // let r = JSON.parse(localStorage.getItem("register_data"));
  // // register_data = JSON.stringify(register_data);
  // for (let i = 0; i <= Registration_data; i++) {
  //   if (Registration_data[i].Mobile == mobile && Registration_data[i].Password == password && Registration_data[i].Email == email) {
  ...
  //   else {
  //     // console.log("YEs")
  //     alert("Registration Registration failed, user already exists or enter valid 10 digit mobile or valid email ");
  //   }
  //   // else if (mobile != "" && password != "" && email != "") {
  //   //   // Registration_data.push(register_data);
  //   //   // localStorage.setItem("register_data", JSON.stringify(Registration_data));
  //   //   // display();
...
//login
let Login_info = JSON.parse(localStorage.getItem("register_data"));
function login() {
  // login_data = {
  ...
  let flag = true;
  let mobile = document.querySelector("#login_mobile_number").value;
  let password = document.querySelector("#one_time_p").value;
  let email = document.querySelector("#email_new").value;
  for (let i = 0; i < Login_info.length; i++) {
    if (Login_info[i].mobile == mobile && Login_info[i].password == password && Login_info[i].email == email) {
      flag = true;
    }
  }
  if (flag == true) {
    alert("Successful Login");
    // localStorage.setItem("user_login", JSON.stringify(Login_info));
    home();
  }
  else {
    alert("Login Failed");
